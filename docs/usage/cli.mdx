...
strix --resume [run-name-or-checkpoint-path]
```
...
<ParamField path="--resume" type="string">
  Resume from a saved checkpoint. Pass a run name such as `app_ab12`, a path to `checkpoint.json`, or omit the value to resume the latest checkpoint.
</ParamField>
